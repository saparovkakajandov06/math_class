  ...
  <div>
    <div class="tasks" v-if="TASKS.length">
      <div class="border-bottom mx-0 row">
        <div class="px-2 col-1">Название задачи</div>
        ...
        <div @click="showFilter(3)" @mouseenter="showFilter(3)" class="px-2 col-1">
        ...
        <div class="px-2 col-1">Источник</div>
        <div class="px-2 col-1">Просмотров/Решено</div>
        <div class="px-2 col text-center">Действия</div>
        <div class="px-2 col text-center">Статус</div>
        <div class="px-2 col-2 text-right">Комментарий администратора</div>
      </div>
      <div
        v-for="task in TASKS"
        v-bind:key="task.id"
        ...
        class="mx-0 row"
      >
        <div class="px-2 col-1">Задача № {{ task.id }}</div>
        ...
        <div class="px-2 col-1"> {{ task.author_name }}</div>
        <div class="px-2 col-1"> {{ task.source_name }}</div>
        <div class="px-2 col-1"> {{ task.views }} / {{ task.resolved }}</div>
        <div class="px-2 col text-center">
          <template v-if="task.status === 'Опубликована'">
            <img class="mr-3" @click.stop="showTaskLink(task.id)" title="скопировать ссылку на эту задачу" src="../assets/img/copy.svg" alt="">
            <img @click.stop="toggleFavoriteTask(task)" :class="task.favorite? '': 'gray_icon'" :title="task.favorite? 'удалить задачу из избранного': 'добавить задачу в избранное'" src="../assets/img/heart_2.svg" alt="">
          </template>
          <p v-else>Ожидается публикации</p>
        </div>
        <div class="px-2 col-1 text-center">
          {{ task.status }}
          <template v-if="task.status === 'Отклонено'">
            <br>
            <a :href="'/tester.html?id=' + task.id" @click="$event.stopPropagation()">Изменить задачу</a>
          </template>
        </div>
        <div class="px-2 col-2 text-right">{{ task.moder_comment }}</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getTasks (page) {
      let url = '/api/get_tasks?my_tasks=yes&page=' + this.page + '&limit=' + this.limit
