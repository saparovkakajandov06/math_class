...
export default {
  ...
  async mounted () {
    ...
    if (isAuthenticated) {
      ...
      const isGet = await this.GET_USER_FAVORITE_TASKS()
      if (isGet) {
        this.getTasks(0)
      }
    } else {
  ...
  methods: {
    ...
    async getTasks (page) {
      let url = '/api/get_tasks?page=' + this.page + '&limit=' + this.limit
      if (this.filters) { url += this.getFilters() } // '&search='+this.search_str;

      this.page = page

      const resultGetTasks = await this.GET_TASKS({ url, page })
      if (resultGetTasks) {
        this.pages = resultGetTasks.pages
        this.show_pages = resultGetTasks.showPages
      }
    },
